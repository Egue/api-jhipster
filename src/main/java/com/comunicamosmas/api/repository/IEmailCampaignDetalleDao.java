...
public interface IEmailCampaignDetalleDao extends CrudRepository<EmailCampaignDetalle , Integer>{
	...
	@Query(value="SELECT contratos.id_contrato , clientes.id_cliente, clientes.mail , contratos.id_servicio FROM controlmas.deudas \n"
			...
			+ "and deudas.id_empresa = :idEmpresa and contratos.preferencia_factura = 2 group by deudas.id_contrato" , nativeQuery=true)
	public List<Object[]> findEmailBySend(@Param("idEmpresa")Long idEmpresa , @Param("fecha") String fecha);
	
	@Query(value="SELECT  \n"
			+ "detalle.id_contrato as idContrato,\n"
	...
	public List<Object[]> findByIdEmailCampaign(@Param("id") Long id);
